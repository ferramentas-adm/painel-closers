...
export async function getAll() {
  const rows = db
    .prepare(
      "SELECT nome, status, status_alterado_em, mesa, alerta_ti, status_origem FROM colaboradores WHERE status IS NOT NULL"
    )
    .all();
  const result = {};
...
export async function setStatus(name, status) {
  ...
  db.prepare(
    "UPDATE colaboradores SET status = ?, status_alterado_em = ?, status_origem = 'manual' WHERE chave = ?"
...
// Retorna { displayName } no sucesso, ou { erro: "email" | "nome" } se ja existir.
export async function createAccount(password, mesa, email, nome) {
  ...
  if (existingEmail) return { erro: "email" };
  ...
  if (existingChave) return { erro: "nome" };
...
export async function applyAutoStatus(nome, status) {
  ...
  closeCurrentPeriod(current.id, current.status, current.status_alterado_em, changedAt);
  db.prepare(
    "UPDATE colaboradores SET status = ?, status_alterado_em = ?, status_origem = 'agenda' WHERE nome = ?"
  ).run(status, changedAt, nome);
}
...
// Retorna false se o email ja pertence a outro colaborador, true no sucesso.
export async function updateColaborador(id, { mesa, email }) {
  const emailTrim = email?.trim() || null;
  if (emailTrim) {
    const outraConta = db
      .prepare("SELECT id FROM colaboradores WHERE lower(email) = ? AND id != ?")
      .get(emailTrim.toLowerCase(), id);
    if (outraConta) return false;
  }
