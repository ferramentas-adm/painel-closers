...
export async function removeCloser(name) {
  db.prepare(
    "UPDATE colaboradores SET status = NULL, status_alterado_em = NULL, alerta_ti = NULL WHERE chave = ?"
  ).run(keyFor(name));
}

export async function clearAll() {
  db.prepare(
    "UPDATE colaboradores SET status = NULL, status_alterado_em = NULL, alerta_ti = NULL"
  ).run();
}

export async function createAccount(name, password, mesa, email) {
  ...
  ).run(chave, displayName, hashPassword(password), mesa.trim(), email?.trim() || null, Date.now());
  return { displayName };
}
...
export async function getColaboradoresComEmail() {
  return db
    .prepare("SELECT nome, email FROM colaboradores WHERE email IS NOT NULL AND email != ''")
    .all();
}
...
export async function applyAutoStatus(nome, status) {
  const changedAt = Date.now();
  db.prepare(
    "UPDATE colaboradores SET status = ?, status_alterado_em = ? WHERE nome = ? AND (status IS NULL OR status != ?)"
  ).run(status, changedAt, nome, status);
}
