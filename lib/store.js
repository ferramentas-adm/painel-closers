...
export async function createAccount(name, password, mesa) {
  ...
  db.prepare(
    "INSERT INTO colaboradores (chave, nome, senha_hash, mesa, criado_em) VALUES (?, ?, ?, ?, ?)"
  ).run(chave, displayName, hashPassword(password), mesa.trim(), Date.now());
  return { displayName };
}
